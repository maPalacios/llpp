...
void ParseScenario::handleAgent(CUDA_DATA *data)
{
  ...
  for (int i = 0; i < n; ++i)
  {
    (data->ax)[getNum+i] = round(x + qrand()/(RAND_MAX/dx) -dx/2);
    (data->ay)[getNum+i] = round(y + qrand()/(RAND_MAX/dy) -dy/2);

    Ped::Tagent *a = new Ped::Tagent(&(data->ax)[getNum+i],&(data->ay)[getNum+i], &(data->wpx)[getNum+i], &(data->wpy)[getNum+i], &(data->wpr)[getNum+i], &(data->lwpx)[getNum+i], &(data->lwpy)[getNum+i], NULL, NULL, &(data->visited)[getNum+i]);
